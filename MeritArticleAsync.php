...
class MeritArticleAsync extends WP_Background_Process
{
    ...
    public static function syncMeritProducts()
    {
        ...
        $settings = MeritClass::getSettings();
        ...
        $background = new MeritArticleAsync();
        ...
        wp_send_json(['message' => "Merit Sync initiated. Synching $syncCount, not synching $noSyncCount. " . time()]);
    }

    protected $action = 'merit_product_import';
    ...
    function task($products)
    {
        ...
        $backOffTime = get_option("merit_backoff_time");
        if ($backOffTime && (time() < $backOffTime)) {
            error_log("Backing up for Merit to honor API limits $backOffTime -> " . time());
            sleep(15);
        ...
        try {
            ...
            error_log("Merit sync error: " . $exception->getMessage() . " " . $exception->getTraceAsString());
            update_option('merit_backoff_time', time() + 5400);
